...
# broj na_vrednosti. I ako je ukljucen argument "benford" vraca rezultat hi-kvadrat test za benforda.
...
# Funkcija "interaktivniPlot" iscrtava inter aktivni plot iz paketa "ggplo2" i "plotly".
#   Argumenti: 
...
# Funkcija "plotovanje" iscrtava grafike histograma, gustine i (opciono) boks-plota, za sve kolone iz prosledjene baze.
#   Argumenti:
...
# Funkcija "primeni_ks_test" primenjuje Kolmogorov-Smirnov test za proveru da li su kolone iz baze normalno raspodeljene i cuva u eksel fajlu.
#   Argumenti:
#     baza - prosledjuje dataset.
#     naziv_baze -  prosledjuje se radi boljeg cuvanja fajla.
...
osnovniPodaciKOLICINA <- function()
{
    ...
    'Gornja granica intervala poverenja_povracaj', 'Koeficijent_povracaj', ' PODACI ZA PRODAJAU',
...
Artikli = merge(Artikli, robne_grupe_ponovo, by = 'NAZIV')
...
baza = Artikli[which(Artikli$KOLICINA > 0),]

aggdata = by(baza[, c("Popravljeni_naziv")], INDICES = baza$RACUN_ID, FUN = paste)
#aggdata = lapply(aggdata, function(x) unique(x))

primeniApriori(aggdata, maks_duzina = 3, nosac = 0.0001, ime = "test_03_11_2017")
primeniEclat(aggdata, nosac = 0.005, min_duzina = 2, maks_duzina = 5, ime = "test_03_11_2017")
...
analiza_fuzzy_klasteri_lojalni_kupci <- function(baza, broj_klastera, nosac_apriori, nosac_eclat, poverenje_apriori, m = 1.8)
{
...
# Primer primene
analiza_fuzzy_klasteri_lojalni_kupci(kmeans_svisemSTAROST_KUPCA, broj_klastera = 8, nosac_eclat = 0.01, nosac_apriori = 0.001, poverenje_apriori = 0.9)

analiza_fuzzy_klasteri_lojalni_kupci(kmeans_svisemSTAROST_KUPCA_I_NAJSKORIJA_KUPOVINA_PRE, broj_klastera = 9, nosac_eclat = 0.01,
...
propustena_prodaja_sve <- function(Artikli, baza, lista_artikal, biraj_minimalno = TRUE, ime)
{
    ...
    for (i in 1 : length(lista_artikal))
    {
        ...
        matrica_propustena_prodaja[i, 1] = paste(ifelse(is.na(artikal[- minimalni][1]), "", artikal[- minimalni][1]), ifelse(is.na(artikal[- minimalni][2]), "", artikal[- minimalni][2]) , ifelse(is.na(artikal[- minimalni][3]), "", artikal[- minimalni][3]), ifelse(is.na(artikal[- minimalni][4]), "", artikal[- minimalni][4]))
        ...
        matrica_propustena_prodaja[i, 5] = round(10 * kolicina1_2_3 / kolicina1_2 *
            (kolicina1_2 - kolicina1_2_3) *
            mean(nova_baza[, 'MP_CENA'])) / 10
    }
    suppressPackageStartupMessages(library(xlsx))
...
#     Argumenti:
#               (grupa, brend, artikal, orgjed, mesec, cena, popust) - ulazni element oblika (robna_grupa, brend, orgjed, mesec, mp_cena, mp_popust)
#               korak_cena - sa kojim korakom kreiramo cene
#               korak_popust - sa kojim korakom kreiramo popust
#               koef - koliko puta idemo standardnom devijacijom u oba smera

# Napomena: mora se ucitati: fact_racuni, artikliLatinica i robne_grupe_ponovo i napraviti odgovarajucu agregaciju za robne grupe.
kombinacije <- function(grupa, brend, artikal, orgjed, mesec, cena, popust, korak_cena, korak_popust, koef)
{

    baza = Artikli[which(Artikli$Popravljeni_naziv == grupa),]

    srednja_cena = as.numeric(cena)
    srednji_popust = as.numeric(popust)


    devijacija_cena = sd(baza$MP_CENA)
    devijacija_popust = sd(baza$POPUST_IZNOS)
    if (is.na(devijacija_cena))
    return(rep(0, 6))

    raspon_cena = c(max(srednja_cena - koef * devijacija_cena, 0), srednja_cena + koef * devijacija_cena)
    raspon_popust = c(max(srednji_popust - koef * devijacija_popust, 0), srednji_popust + koef * devijacija_popust)

    #print(raspon_cena)


    vektor_cena = seq(from = raspon_cena[1], to = raspon_cena[2], by = korak_cena)
    vektor_popust = seq(from = raspon_popust[1], to = raspon_popust[2], by = korak_popust)

    # sad ide outer i tako to
    # grid_kombinacija = vektor_cena %o% vektor_popust
    grid_kombinacija = expand.grid(vektor_cena, vektor_popust)
    colnames(grid_kombinacija) = c('CENA', 'POPUST')

    n = length(grid_kombinacija[[1]])
    matrica = matrix(ncol = 7, nrow = n)
    matrica[, 1] = rep(grupa, times = n)
    matrica[, 2] = rep(brend, times = n)
    matrica[, 3] = rep(artikal, times = n)

    matrica[, 4] = rep(orgjed, times = n)
    matrica[, 5] = rep(mesec, times = n)
    matrica[, 6] = grid_kombinacija[[1]]
    matrica[, 7] = grid_kombinacija[[2]]
